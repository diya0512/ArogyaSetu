  ...
  return (
    <div style={{ background: "#f7f8fa" }}>
      <style>{`
        ...
        .trust-strip { display: flex; gap: 32px; }
        .stats-grid { display: grid; grid-template-columns: repeat(4,1fr); }
        .services-grid { display: grid; grid-template-columns: repeat(2,1fr); gap: 1px; background: #e2e8f0; border: 1px solid #e2e8f0; border-radius: 10px; overflow: hidden; }
        .how-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 80px; align-items: center; }
        .schemes-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 12px; }
        .cta-grid { display: grid; grid-template-columns: 1fr auto; gap: 32px; align-items: center; }
        .emergency-inner { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 16px; }
        .service-card { background: #fff; padding: 32px; display: block; transition: background 0.15s; text-decoration: none; }
        ...
        @media (max-width: 768px) {
          ...
          .schemes-grid { grid-template-columns: repeat(2,1fr) !important; }
          .cta-grid { grid-template-columns: 1fr !important; }
          .emergency-inner { flex-direction: column; align-items: flex-start !important; }
      ...
      <section style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "64px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }} className="hero-grid">
          <div>
            ...
            <h1 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(32px, 5vw, 48px)", color: "#0f2d52", lineHeight: 1.1, marginBottom: 20, fontWeight: 400 }}>
              Free healthcare<br />
              <em style={{ fontStyle: "italic", color: "#c6902a" }}>for every</em><br />
              Indian citizen.
            </h1>
            <p style={{ fontSize: 15, color: "#6b7280", lineHeight: 1.75, marginBottom: 32, maxWidth: 440 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <Link href="/hospitals" className="btn-primary">Find a hospital</Link>
              <Link href="/appointments" className="btn-outline">Book appointment</Link>
              <Link href="/disease-tracker" className="btn-ghost">Disease tracker</Link>
            </div>
            <div style={{ marginTop: 40, paddingTop: 28, borderTop: "1px solid #f1f5f9" }} className="trust-strip">
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }} className="stats-grid">
          {stats.map((s, i) => (
            <div key={s.label} style={{ textAlign: "center", padding: "12px 16px", borderRight: i < 3 ? "1px solid rgba(255,255,255,0.1)" : "none" }}>
              ...
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 5 }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: "72px 24px", background: "#f7f8fa" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(24px, 4vw, 34px)", color: "#0f2d52", fontWeight: 400, maxWidth: 400, lineHeight: 1.2 }}>One platform for all government healthcare</h2>
          </div>
          <div className="services-grid">
            {services.map(s => (
              <Link key={s.title} href={s.href} className="service-card">
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
                  <div style={{ width: 40, height: 40, background: "#f0f4fa", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", color: "#0f2d52" }}>{s.icon}</div>
      ...
      <section style={{ padding: "72px 24px", background: "#fff", borderTop: "1px solid #e2e8f0", borderBottom: "1px solid #e2e8f0" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }} className="how-grid">
          <div>
            ...
            <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(24px, 4vw, 34px)", color: "#0f2d52", fontWeight: 400, lineHeight: 1.2, marginBottom: 16 }}>
              Book a government hospital appointment in minutes
            </h2>
            <p style={{ color: "#6b7280", fontSize: 14, lineHeight: 1.75, marginBottom: 32 }}>No middlemen, no queues, no costs. Direct access to government doctors across India.</p>
            <Link href="/appointments" className="btn-primary">Book free appointment</Link>
          </div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            {steps.map((s, i) => (
              <div key={s.n} style={{ display: "flex", gap: 20, paddingBottom: i < steps.length - 1 ? 28 : 0, position: "relative" }}>
                {i < steps.length - 1 && <div style={{ position: "absolute", left: 16, top: 36, width: 1, height: "calc(100% - 8px)", background: "#e2e8f0" }} />}
                <div style={{ width: 33, height: 33, borderRadius: "50%", background: "#0f2d52", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontFamily: "'DM Serif Display', serif", fontSize: 15, color: "#fff", zIndex: 1 }}>{s.n}</div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 36, flexWrap: "wrap", gap: 16 }}>
            <div>
              ...
              <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(24px, 4vw, 34px)", color: "#0f2d52", fontWeight: 400, lineHeight: 1.2 }}>Government health schemes</h2>
            </div>
            <Link href="/dashboard" style={{ fontSize: 13, fontWeight: 600, color: "#0f2d52", textDecoration: "none", border: "1px solid #c3d0e8", padding: "7px 16px", borderRadius: 5, background: "#fff" }}>View all schemes →</Link>
          </div>
      ...
      <section style={{ background: "#0f2d52", padding: "72px 24px" }}>
        <div style={{ maxWidth: 700, margin: "0 auto" }} className="cta-grid">
          <div>
            <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: "clamp(24px, 4vw, 32px)", color: "#fff", fontWeight: 400, marginBottom: 10, lineHeight: 1.2 }}>
              Healthcare is your right.<br /><em style={{ color: "#c6902a" }}>Use it.</em>
            </h2>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.55)", lineHeight: 1.7 }}>Crores of citizens already access free government healthcare through ArogyaSetu.</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, flexShrink: 0 }}>
            <Link href="/appointments" style={{ background: "#c6902a", color: "#fff", fontWeight: 700, padding: "12px 24px", borderRadius: 6, fontSize: 14, textDecoration: "none", textAlign: "center" }}>Book appointment</Link>
            <Link href="/auth" style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.8)", fontWeight: 500, padding: "12px 24px", borderRadius: 6, fontSize: 14, textDecoration: "none", textAlign: "center", border: "1px solid rgba(255,255,255,0.12)" }}>Create account</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: "#7f1d1d", padding: "20px 24px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }} className="emergency-inner">
